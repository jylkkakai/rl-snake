...
#                       MAIN                            #
#########################################################
game = False # False to disable the game animation
game_screen = None
...
height = 30
episodes = prev_epochs + 10001 # Number of epochs is trained
batch_size = 32
state_size = 12
action_size = 4
max_score = 0
...
for e in range(prev_epochs ,episodes):
    ...
    while True or exit:
        ...
        if score > max_score:
            max_score = score
        if game:
            exit = game_window.draw_screen([snakes.get_snake(), snakes.get_food(), e, score, max_score])
            time.sleep(0.005)
            if exit:
                break
        ...
        if done or steps > 200:
            print("episode: {}/{}, iterations: {}, reward_sum: {}, score: {}, eps: {}".format(e, episodes, step_counter, sum_reward, score, agent.get_eps()))
            break
        steps += 1
        step_counter += 1
